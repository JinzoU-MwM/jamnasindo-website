...
install_runner() {
  ...
  read -rp "   Paste runner registration token (blank to skip): " token
  if [ -z "$token" ]; then
    warn "Skipping runner install — re-run this script later to add it."
    return
  fi
  local dir="$HOME/actions-runner"
  mkdir -p "$dir"; cd "$dir"
  if [ ! -f ./config.sh ]; then
    local ver
  ...
  ./config.sh --url "$GITHUB_REPO_URL" --token "$token" \
    --name "$(hostname)" --labels self-hosted,linux,x64 \
